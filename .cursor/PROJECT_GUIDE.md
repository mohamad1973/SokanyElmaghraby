...
| ارتباط صور المكتبة (فلتر/CSV) | Code Snippets ← `wordpress-plugin/snippets/sokany-media-usage-badge-v1.2.php` (عطّل v1.1/v1.0) |
...
|---------|-----------|----------------|
| CS تعديل قيمة الفاتورة | زائد أو ناقص على كارت قيمة الفاتورة يغيّر الإجمالي فوراً، وكارت الديبوزت يظهر من 5000 ويختفي تحتها، والفرق يتحفظ في orderTotalDelta | `call-sheet.tsx`, `confirmations.ts`, `agents.ts` |
| CS شحن للأدمن | قائمة شحن في هيدر الأدمن تفتح شيت سيد تميمة وتوزيع المناديب، والقائمة تثبت جوه الشاشة على الموبايل | `cs-shell.tsx`, `cs/temima/page.tsx`, `courier-dispatch/route.ts` |
| CS وقت ديبوزت واسم انستا | وقت الدفع يُكتب يدوياً مع صباحاً ومساءً، واسم حساب انستا اختياري ولا يمنع الحفظ أو الإشعار | `call-sheet.tsx`, `deposit-approvals.ts`, `order-notification-bar.tsx` |
| CS فلتر تاريخ تميمة | أوردرات تميمة المحفوظة تتفلتر بتاريخ إنشاء الأوردر أو أول تاريخ حفظ، وأول حفظ يثبت وتعديل الحفظ يتسجل لوحده | `confirmations.ts`, `cs-queue-client.tsx`, `call-sheet.tsx` |
| CS حالات تسليم تميمة | مشرف المناديب يفتح على مؤكد طباعة تميمة لليوم، ويقدر يفلتر تم التسليم والرفض والتأجيل ولم يوزع وبعد آخر توزيع. المندوب يعلّم التسليم أو الرفض بسبب أو التأجيل | `cs-queue-client.tsx`, `courier-dispatch.ts`, `couriers-client.tsx` |
| CS بدون سقف 500 | قائمة الأوردرات تحمّل كل صفوف آخر 30 يوم، وفلتر من/إلى يعرض الكل في المدى المختار | `confirmations.ts` |
| CS توزيع مناديب تميمة | صلاحية مشرف مناديب وصلاحية مندوب؛ توزيع أوردرات تميمة المؤكدة اليوم حسب منطقة الكارت، وتعليم التسليم يعلّم تسليم العميل والشحن ويترك متابعة العميل | `agents.ts`, `courier-dispatch.ts`, `cs/couriers`, `users-client.tsx` |
| CS إخفاء الكمية صفر | تشيك بوكس في التحويلات يخفي الأصناف الصفرية من المخزون ومن تقرير الأونلاين والعاشر | `transfers-client.tsx`, `warehouse-transfer-report.ts` |
| CS تحويل المخازن | رفع أرصدة الأونلاين والعاشر والعاشر المنزلي وتقرير الأصناف تحت حد الطلب اللي ينفع تتحول | `warehouse-transfer-report.ts`, `transfers/warehouses/route.ts`, `transfers-client.tsx` |
| CS إعادة حفظ البوليصة | بعد تم الحفظ، حفظ المتابعة يكتب الاسم والتليفون والعنوان على الكارت ويحدّث نفس بوليصة بوسطة | `confirmations.ts`, `bosta-waybill.ts`, `call-sheet.tsx` |
| CS تحديث بوليصة بوسطة | حفظ العنوان أو التليفون يحدّث نفس البوليصة بمدينتها ومنطقتها الحاليين وتليفون بصيغة 01، ورسالة الرفض تظهر على الكارت | `bosta-client.ts`, `call-sheet.tsx` |
| CS مرجع بوسطة Woocommerce_ | طلب واحد يبحث عن Woocommerce_ ورقم الأوردر ويجيب البوليصة من غير تقليب صفحات | `bosta-client.ts`, `bosta-waybill.ts` |
| CS الرقم المرجعي لبوسطة | مقارنة رقم الأوردر بالرقم المرجعي على البوليصة، وعند التطابق تنزل بيانات الشحنة كاملة من غير بحث بالموبايل | `bosta-client.ts`, `bosta-waybill.ts` |
| CS خطأ كارت بوسطة | إيقاف طلب قائمة الشحنات المرفوض، وإخفاء صفحة الخطأ، وقراءة رقم التراك المحفوظ على أوردر ووكومرس | `bosta-client.ts`, `bosta-waybill.ts`, `orders.ts` |
| CS ربط بوليصة بوسطة برقم الأوردر | نزول الأوردر من الموقع يبحث في بوسطة بنفس رقم الأوردر ويكتب التراك والحالة وقيمة الشحن على صف القائمة | `bosta-client.ts`, `bosta-waybill.ts`, `confirmations.ts`, `cs-queue-client.tsx` |
| CS إظهار بوليصة بوسطة | البحث يقرأ أشكال رد بوسطة ويجيب البوليصة الكاملة؛ الكارت يعرض التراك والحالة والشحن والتحصيل أو سبب الفشل | `bosta-client.ts`, `bosta-waybill.ts`, `call-sheet.tsx`, `bosta-zones.ts` |
| CS ربط بوليصة بوسطة الموجودة | لو الشحنة موجودة على بوسطة بنفس الموبايل، فتح الأوردر ينزل رقم التراك والحالة وقيمة الشحن بدل «لسه مفيش بوليصة» | `bosta-waybill.ts`, `bosta-client.ts`, `call-sheet.tsx` |
| CS بوليصة بوسطة | تم الحفظ ينشئ البوليصة وينزل التراك وقيمة الشحن والحالة والتحصيل وآخر حركة؛ صف القائمة يعرض شحن بوسطة؛ تعديل العنوان والتليفون يتحدث على بوسطة | `bosta-waybill.ts`, `bosta-client.ts`, `call-sheet.tsx`, `cs-queue-client.tsx` |
| CS أدمن كامل + سرعة المكالمة | الأدمن معه كل الصلاحيات ورقم الفاتورة؛ اسم الموظفة ظاهر؛ تاريخ من/إلى يفتح من أي ضغطة؛ صفحة المكالمة تفتح فوراً وتحديث ووكومرس بعدها؛ فورم المستخدمين صف حقول والصلاحيات تحته | `agents.ts`, `cs-queue-client.tsx`, `call-sheet.tsx`, `users-client.tsx` |
| CS صلاحيات متعددة | الموظف يجمع أكثر من صلاحية؛ حسابات ترى كل الأوردرات وتكتب رقم الفاتورة | `agents.ts`, `users-client.tsx`, `cs-queue-client.tsx` |
| CS رقم فاتورة في القائمة والطباعة | خانة 10 أرقام بين اسم الموظفة والسعر للمشرفة والأدمن والحسابات؛ عمود الفاتورة في شيت تميمة وبوستة | `cs-queue-client.tsx`, `confirmations.ts` |
| CS إلغاء كارت أمر البيع | إزالة كارت رقم أمر البيع من صفحة الأوردر وعموده من شيت تميمة | `call-sheet.tsx`, `cs-queue-client.tsx` |
| CS موظف حسابات + رقم فاتورة | دور accounting يرى كل الأوردرات بدون توزيع؛ عند فلتر تم الحفظ تكست بوكس رقم فاتورة يُحفظ تلقائياً | `agents.ts`, `cs-queue-client.tsx`, `confirmations.ts`, `users-client.tsx` |
