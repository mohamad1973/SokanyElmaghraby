/**
 * Snippet Name: SOKANY Cart Checkout Style v1.2
 * Description: v1.2 — عرض 80% من الشاشة على الديسكتوب، عمودين قسريين (سلة + تشيك أوت)، إخفاء البلد، نص أسود في Select2 + حفظ بيانات.
 *
 * Code Snippets:
 * - Do NOT paste a <?php opening tag
 * - Run snippet: Only run on site front-end
 * - Deactivate/delete v1.0 and v1.1 first
 * - Activate this snippet
 *
 * Source: wordpress-plugin/snippets/sokany-cart-checkout-style-v1.2.php
...
function sokany_cc12_is_target_page(): bool {
...
add_action('wp_enqueue_scripts', function () {
    if (!sokany_cc12_is_target_page()) {
        return;
    }

    wp_register_style('sokany-cc12-style', false, [], '1.2.0');
    wp_enqueue_style('sokany-cc12-style');
    wp_add_inline_style('sokany-cc12-style', sokany_cc12_css());

    wp_register_script('sokany-cc12-script', false, [], '1.2.0', true);
    wp_enqueue_script('sokany-cc12-script');
    wp_add_inline_script('sokany-cc12-script', sokany_cc12_js(), 'after');
}, 99);

function sokany_cc12_css(): string {
    return <<<'CSS'
:root {
  --sokany-lime: #daff00;
...
function sokany_cc12_js(): string {
    return <<<'JS'
(function () {
