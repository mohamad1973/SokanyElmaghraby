/**
 * Snippet Name: SOKANY Product Compare v1.1.1
 * Description: إصلاح ظهور أيقونة المقارنة داخل هيدر الثيم قرب البحث + مزامنة زر المقارنة الأصلي + صفحة مقارنة موحدة.
 *
 * Code Snippets:
 * - Do NOT paste a <?php opening tag
 * - Run snippet: Only run on site front-end
 * - Deactivate Product Compare v1.1 first
 * - Activate this snippet
 * - بعد أول تفعيل: Settings -> Permalinks -> Save مرة واحدة لتفعيل /compare-products/
 *
 * Source: wordpress-plugin/snippets/sokany-product-compare-v1.1.1.php
...
const SOKANY_CMP111_STORAGE_KEY = 'sokany-wp-compare-v1';
const SOKANY_CMP111_MAX_ITEMS = 4;
const SOKANY_CMP111_PAGE_SLUG = 'compare-products';

function sokany_cmp111_is_woo_ready(): bool {
    return class_exists('WooCommerce') && function_exists('wc_get_product');
}

function sokany_cmp111_is_compare_page(): bool {
    ...
    return $path === SOKANY_CMP111_PAGE_SLUG;
}

add_action('init', function () {
    add_rewrite_rule('^' . SOKANY_CMP111_PAGE_SLUG . '/?$', 'index.php?sokany_compare=1', 'top');
});
...
add_action('rest_api_init', function () {
    register_rest_route('sokany-compare/v1', '/products', [
        ...
        'callback' => 'sokany_cmp111_rest_products',
        'permission_callback' => '__return_true',
    ]);
});

function sokany_cmp111_rest_products(WP_REST_Request $request) {
    if (!sokany_cmp111_is_woo_ready()) {
    ...
    $ids = array_slice($ids, 0, SOKANY_CMP111_MAX_ITEMS);
    ...
    foreach ($ids as $id) {
        ...
        $products[] = [
            ...
            'specs' => sokany_cmp111_collect_specs($product),
        ];
    }
...
function sokany_cmp111_collect_specs(WC_Product $product): array {
    $specs = [];
...
add_action('template_redirect', function () {
    if (!sokany_cmp111_is_compare_page()) {
        return;
    }
    status_header(200);
    nocache_headers();
    echo sokany_cmp111_render_compare_page();
    exit;
}, 1);

function sokany_cmp111_render_compare_page(): string {
  ...
  <main class="sokany-cmp-wrap">
    <div class="sokany-cmp-inner">
      ...
      <div id="sokany-cmp-app"
           data-max-items="<?php echo esc_attr((string) SOKANY_CMP111_MAX_ITEMS); ?>"
...
add_action('wp_enqueue_scripts', function () {
    ...
    wp_register_style('sokany-cmp111-style', false, [], '1.1.1');
    wp_enqueue_style('sokany-cmp111-style');
    wp_add_inline_style('sokany-cmp111-style', sokany_cmp111_css());

    wp_register_script('sokany-cmp111-script', false, [], '1.1.1', true);
    wp_enqueue_script('sokany-cmp111-script');
    wp_add_inline_script('sokany-cmp111-script', sokany_cmp111_js(), 'after');
}, 30);

function sokany_cmp111_css(): string {
    return <<<'CSS'
:root{
  --cmp-gold:#daff00;
  --cmp-border:rgba(0,0,0,.1);
}
.sokany-cmp-header-slot{display:inline-flex;align-items:center;position:relative}
.sokany-cmp-header-link{
  ...
  margin-inline-end:8px;z-index:50;
}
...
function sokany_cmp111_js(): string {
    $compare_url = home_url('/' . SOKANY_CMP111_PAGE_SLUG . '/');
    $max_items = (int) SOKANY_CMP111_MAX_ITEMS;
    return "(function(){\n" .
      "var KEY='" . esc_js(SOKANY_CMP111_STORAGE_KEY) . "';\n" .
      "var MAX=" . $max_items . ";\n" .
      ...
      "function attachBefore(anchor,icon){if(!anchor||!icon)return false;var host=anchor.closest('.whb-column,.whb-col,.wd-header-search,.searchform,.elementor-widget-search-form,.header-search-form')||anchor.parentElement;if(!host||!host.parentNode)return false;if(icon.parentNode)icon.parentNode.removeChild(icon);host.classList.add('sokany-cmp-header-slot');host.parentNode.insertBefore(icon,host);notify();return true;}\n" .
      "function ensureHeaderIcon(){var icon=document.querySelector('.sokany-cmp-header-link');if(!icon){icon=document.createElement('a');icon.href=COMPARE_URL;icon.className='sokany-cmp-header-link';icon.setAttribute('aria-label','مقارنة المنتجات');icon.setAttribute('title','مقارنة المنتجات');icon.innerHTML='<svg viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.9\"><path d=\"M7 5v14M7 19l-2-2M7 19l2-2M17 19V5M17 5l-2 2M17 5l2 2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"/></svg><span class=\"sokany-cmp-count\" style=\"display:none\">0</span>';}\n" .
      "var searchTargets=['.whb-header .wd-search-form','.whb-header form.woocommerce-product-search','.whb-header form[role=\"search\"]','.whb-header input[type=\"search\"]','.wd-header-search form','.header-search-form form','.site-header .search-form','.site-header form[role=\"search\"]','.ast-header-search','.elementor-search-form'];\n" .
      "for(var i=0;i<searchTargets.length;i++){var node=document.querySelector(searchTargets[i]);if(!node)continue;if(node.tagName==='INPUT'){node=node.closest('form')||node;}if(attachBefore(node,icon))return;}\n" .
      "var headerTargets=['.whb-column-right','.whb-col-right','.whb-general-header .container .row','.whb-general-header .whb-row','.whb-header .whb-row','.whb-header .container','.wd-header-content','.site-header .header-inner','.site-header .container','header .container'];\n" .
      "for(var j=0;j<headerTargets.length;j++){var holder=document.querySelector(headerTargets[j]);if(!holder)continue;if(icon.parentNode!==holder){holder.insertBefore(icon,holder.firstChild);}notify();return;}\n" .
      "var firstHeader=document.querySelector('header');if(firstHeader){firstHeader.insertBefore(icon,firstHeader.firstChild);notify();}\n" .
      "}\n" .
