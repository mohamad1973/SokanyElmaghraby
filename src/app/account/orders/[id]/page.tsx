    ...
    <div className="py-12">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-4xl font-bold text-zinc-950">طلب #{order.number}</h1>
            <p className="mt-2 text-sm text-zinc-600">الحالة الحالية: {order.status}</p>
          </div>
        ...
        <section className="rounded-[2rem] bg-white p-6 shadow-sm">
          <h2 className="text-2xl font-bold text-zinc-950">تتبع الطلب</h2>
        ...
        <section className="rounded-[2rem] bg-white p-6 shadow-sm">
          <h2 className="text-2xl font-bold text-zinc-950">منتجات الطلب</h2>
            ...
            {order.items.map((item) => (
              <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-zinc-50 p-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-zinc-500">SKU: {item.sku || "غير محدد"}</p>
                </div>
                <p className="text-sm font-bold text-zinc-700">الكمية: {item.quantity}</p>
